...
export const create = api<CreateAnnouncementRequest, Announcement>(
  ...
  async (req) => {
    console.log('####create anouncement', req);
    const auth = getAuthData()!;
    const id = req.createdBy || auth?.id;
    console.log('####create announcement by user id:', id, auth.id);
    ...
    const announcement = await db.queryRow<Announcement>`
      ...
      VALUES (${req.title}, ${req.content}, ${imageUrl || req.imageData || null}, ${id})
